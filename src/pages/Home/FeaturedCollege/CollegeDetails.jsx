...
const CollegeDetails = () => {
  ...
  return (
      ...
      <div className="mx-auto w-full max-w-7xl px-5 py-12 md:px-10 md:py-16 lg:py-20">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:gap-10">
          ...
          <div className="flex flex-col gap-8 lg:w-3/5">
            <h2 className=" text-3xl font-bold md:text-5xl font-sans drop-shadow-md">{college.name}</h2>
            <div className='flex flex-col md:flex-row gap-5 '>
            <p className="text-sm sm:text-xl font-semibold  drop-shadow-md " >
            Admision Process : <span className='text-base'> {college?.admission_process.process}</span>
            <div className='flex items-center flex-wrap gap-10'>
         <div>
         <p className='font-sans text-base mt-10'>Requirements</p>
            <hr className='w-28'/>
            {
                college?.admission_process.requirements.map((x, i) =>  <span key={i} className="text-[14px] my-1 flex items-center gap-2"><FaLocationArrow className='text-lime-500'/>{x}</span>)
            }
         </div>
    ...
    <hr className='w-28'/>
    <span  className="text-[14px] my-1 flex items-center gap-2"><MdOutlineDateRange className='text-lime-500'/><span className='text-gray-500'>Early Decision</span>: {college?.admission_process.deadlines.early_decision}</span>

    <span  className="text-[14px] my-1 flex items-center gap-2"><MdOutlineDateRange className='text-lime-500'/><span className='text-gray-500'>Regular Decision</span>: {college?.admission_process.deadlines.regular_decision}</span>
    <span  className="text-[14px] my-1 flex items-center gap-2"><MdOutlineDateRange className='text-lime-500'/><span className='text-gray-500'>Financial Aid</span>: {college?.admission_process.deadlines.financial_aid}</span>
            ...
            <div className='flex gap-5 flex-wrap '>
                {
                    college?.events.map((x, i) =>     <div key={i}
            ...
            <div className="grid gap-8 md:grid-cols-2 md:gap-4 -mt-8">
                {
                    college?.research_works.map((x, i) => <div key={i}
...
{
    college?.sports_categories.map((x, i) =>   <button key={i} title="Save " className="cursor-pointer flex items-center fill-lime-400 bg-lime-950 hover:bg-lime-900 active:border active:border-lime-400 rounded-md duration-100 px-2 py-2">
    ...
    <span className="text-sm text-lime-400 font-bold pr-1">{x}</span>
          ...
          {/* Image */}
          <div className="w-full rounded-md  max-[991px] lg:w-2/5"><img className=' rounded-md' src={college?.images} alt="" /></div>
        </div>
      </div>
    </section>
    </div>
